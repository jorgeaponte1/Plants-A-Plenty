...
const updatePlant = expressHandler(async (req, res) => {
  ...
  if (!plant) {
    res
      .status(500)
      .json({ message: `Plant not found for id: ${req.params.id}` });
  }
...
const getOnePlant = expressHandler(async (req, res) => {
  ...
  if (!plants) {
    res
      .status(500)
      .json({ message: `There are no plants with the id: ${req.params.id}` });
  }
  res.status(200).json(plants);
});
...
const addItemToCart = expressHandler(async (req, res) => {
  ...
  if (!plant) {
    res
      .status(500)
      .json({ message: `Plant not found with id ${req.params.id}` });
  }

  // Find the cart item with the same plant ID
  const cartItem = user.cart.find(
    (item) => item.plant.toString() === plant._id.toString()
  );
...
const successOrder = expressHandler(async (req, res) => {
  res.send(
    `<html><body><h1>Congratulations! Your order has been successfully processed and payment has been received. Thank you for choosing Plants A Plenty</h1></body></html>`
  );
});
